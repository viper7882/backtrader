        ...
        if self.p.printdata:
            self.log(('Time used: %s  - Period % d - '
                     ...
                     (str(tused), self.p.period,
                      self.broker_or_exchange.starting_cash, self.broker_or_exchange.get_value()))

        value = '%.2f' % self.broker_or_exchange.get_value()
        _chkvalues.append(value)

        cash = '%.2f' % self.broker_or_exchange.get_cash()
        _chkcash.append(cash)

    def next(self):
        # print('self.datafeed.close.array:', self.datafeed.close.array)
        if self.orderid:
