...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.sma = btind.SMA(self.datafeed, period=self.p.period)
        self.cross = btind.CrossOver(self.datafeed.close, self.sma, plot=True)
    ...
    def start(self):
        ...
        if self.p.printdata:
            ...
            self.log('Starting portfolio value: %.2f' % self.broker_or_exchange.get_value(),
                     nodate=True)
    ...
    def stop(self):
        ...
        if self.p.printdata:
            ...
            if not self.p.stocklike:
                assert '%.2f' % self.broker_or_exchange.get_value() == '12795.00'
                assert '%.2f' % self.broker_or_exchange.get_cash() == '11795.00'
            else:
                assert '%.2f' % self.broker_or_exchange.get_value() == '10284.10'
                assert '%.2f' % self.broker_or_exchange.get_cash() == '6164.16'
    ...
    def next(self):
        if self.p.printdata:
            ...
            self.log('Close %.2f - Sma %.2f' %
                     (self.datafeed.close[0], self.sma[0]))
        ...
        if not self.position.size:
            if self.cross > 0.0:
                if self.p.printops:
                    self.log('BUY CREATE , %.2f' % self.datafeed.close[0])

                self.orderid = self.buy()
                chkprice = '%.2f' % self.datafeed.close[0]
                self.buycreate.append(chkprice)

        elif self.cross < 0.0:
            if self.p.printops:
                self.log('SELL CREATE , %.2f' % self.datafeed.close[0])
