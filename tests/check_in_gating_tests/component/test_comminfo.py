...
def check_stocks():
    ...
    opcost = comm.get_operating_cost(size=size, price=price)
    assert opcost == size * price

    pos = Position(size=size, price=price)
    value = comm.get_value(pos, price)
    assert value == size * price

    commcost = comm.get_commission_rate(size, price)
    assert commcost == size * price * commission

    newprice = 5.0
    pnl = comm.profit_and_loss(pos.size, pos.price, newprice)
    assert pnl == pos.size * (newprice - price)

    ca = comm.cash_adjust(size, price, newprice)
    assert not ca
